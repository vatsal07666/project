...
import {
  ...
  TableHead, TableRow, Tooltip, InputBase, CircularProgress,
...
const ProductCard = ({ item, index, onEdit, onDelete }) => (
  <Paper sx={{ p: 2, mb: 2 }}>
    <Box sx={{ display: "flex", justifyContent: "space-between" }}>
      ...
      <Box>
        <IconButton onClick={() => onEdit(item)}><FaEdit /></IconButton>
        <IconButton onClick={() => onDelete(item._id, index)}><RiDeleteBin6Line /></IconButton>
      </Box>
    </Box>
    <Divider sx={{ my: 1 }} />
    <Box sx={{ fontSize: 14 }}>
      ...
      <div><b>Cost:</b> ₹{item.costPrice}</div>
      <div><b>Selling:</b> ₹{item.sellingPrice}</div>
    </Box>
  </Paper>
);
...
export default function AddProduct() {
  ...
  const handleSubmit = (values, { resetForm }) => {
    const api = editId
      ? axios.patch(`https://generateapi.techsnack.online/api/product/${editId}`, values, { headers: { Authorization: token } })
      : axios.post("https://generateapi.techsnack.online/api/product", values, { headers: { Authorization: token } });

    api.then(() => {
      ShowSnackbar(editId ? "Updated!" : "Added!", "success");
      getData();
      resetForm();
      setOpen(false);
      setEditId(null);
  ...
  return (
    <Paper sx={{ p: 3, mt: 10 }}>
      ...
      <Box sx={{ display: "flex", flexDirection: { xs: "column", md: "row" }, gap: 2, justifyContent: "space-between" }}>
        <h1>Products</h1>
        <Button startIcon={<IoMdAdd />} onClick={() => setOpen(true)}>Add Product</Button>
      </Box>

      {/* SEARCH */}
      <Box sx={{ mt: 2, width: { xs: "100%", md: 300 }, position: "relative" }}>
        <InputBase
          fullWidth
          placeholder="Search Product"
          value={searchItem}
          onChange={(e) => setSearchItem(e.target.value)}
          sx={{ pl: 5, border: 1, borderRadius: 2 }}
        />
        <SearchIcon sx={{ position: "absolute", left: 10, top: "50%", transform: "translateY(-50%)" }} />
      </Box>
        ...
        {isMobile ? (
          filtered.map((item, i) => (
            <ProductCard key={item._id} item={item} index={i}
              onEdit={(i) => { setFormValues(i); setEditId(i._id); setOpen(true); }}
              onDelete={(id) => axios.delete(`https://generateapi.techsnack.online/api/product/${id}`, { headers: { Authorization: token } }).then(getData)}
            />
          ))
        ) : (
          <TableContainer>
            <Table>
              <TableHead sx={{ bgcolor: "#1e293b" }}>
                <TableRow>
                  {["#", "Product", "SKU", "Category", "Supplier", "Stock", "Cost", "Selling", "Actions"]
                    .map(h => <TableCell key={h} sx={{ color: "#fff" }}>{h}</TableCell>)}
                </TableRow>
              </TableHead>
              <TableBody>
                {filtered.map((item, i) => (
                  <TableRow key={item._id}>
                    ...
                    <TableCell>
                      <IconButton onClick={() => { setFormValues(item); setEditId(item._id); setOpen(true); }}><FaEdit /></IconButton>
                      <IconButton onClick={() => axios.delete(`https://generateapi.techsnack.online/api/product/${item._id}`, { headers: { Authorization: token } }).then(getData)}><RiDeleteBin6Line /></IconButton>
                    </TableCell>
                  </TableRow>
                ))}
              </TableBody>
      ...
      {/* FORM */}
      <Dialog open={open} fullScreen={isMobile} onClose={() => setOpen(false)}>
        <DialogTitle>{editId ? "Edit Product" : "Add Product"}</DialogTitle>
        <DialogContent>
          <Formik initialValues={formValues} enableReinitialize validationSchema={validationSchema} onSubmit={handleSubmit}>
            <Form>
              {["productName", "sku", "stock", "costPrice", "sellingPrice"].map(f => (
                <Field key={f} name={f} placeholder={f} />
              ))}
              <DialogActions>
